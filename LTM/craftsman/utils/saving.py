...
class SaverMixin:
    ...
    def save_mesh(self, filename, v_pos, t_pos_idx, v_tex=None, t_tex_idx=None, v_color = None) -> str:
        save_path = self.get_save_path(filename)
        v_pos = self.convert_data(v_pos)
        t_pos_idx = self.convert_data(t_pos_idx)
        mesh = trimesh.Trimesh(vertices=v_pos, faces=t_pos_idx,process=False)
        mesh.visual.vertex_colors = v_color
        mesh.export(save_path)
        return save_path


    def save_xyz_normal_points(self, filename, points, colors=None) -> str:
        save_path = self.get_save_path(filename)
        if points.shape[-1] == 6:
            self.save_vertices_as_ply_open3d(points.detach().cpu().numpy()[:,:3], colors.detach().cpu().numpy(), points.detach().cpu().numpy()[:,3:], save_path)
        else:
            self.save_vertices_as_ply_open3d(points.detach().cpu().numpy()[:,:3], colors.detach().cpu().numpy(), None, save_path)
        return save_path
    

    def save_vertices_as_ply_open3d(self, vertices, colors, normal, filepath):
    ...
    # def save_qual_latents(self, filename, plane_res) -> str:
    #     save_path = self.get_save_path(filename)
    #     v_pos = self.convert_data(v_pos)
    #     t_pos_idx = self.convert_data(t_pos_idx)
