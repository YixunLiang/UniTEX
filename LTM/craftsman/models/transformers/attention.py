...
class MultiheadCrossAttention(nn.Module):
    ...
    def forward(self, x, data,pos_emb=None):
        if pos_emb is not None:
            x = self.c_q(x) + pos_emb
        else:
            x = self.c_q(x)
        data = self.c_kv(data)
...
class ResidualCrossAttentionLoRABlock(nn.Module):
    def __init__(
        self,
        *,
        n_data: Optional[int] = None,
        ...
        if down_sample:
            self.downsample = nn.Conv2d(width,width,2,2,0,bias=False)
            self.upsample = nn.PixelShuffle(2)
            self.conv_out = nn.Conv2d(width//(2*2),width,1,1,0,bias=False)
        else:
            self.downsample = nn.Identity()
            self.upsample = nn.Identity()
            self.conv_out = nn.Conv2d(width,width,1,1,0,bias=False)
    ...
    def imgs2seqs(self,x):
        return rearrange(x,"B C H W -> B (H W) C").contiguous()

    def seqs2imgs(self,x,h,w):
        return rearrange(x,"B (H W) C -> B C H W",H=h,W=w).contiguous()

    def forward(self, x: torch.Tensor, data: torch.Tensor, pos_emb=None,plane_num=3):
        # h_org = int((x.shape[1]//plane_num)**0.5)
        # x = self.seqs2imgs(x,h=h_org,w=plane_num*h_org)
        x = self.downsample(x)
        B,C,H,W = x.shape
        x = self.imgs2seqs(x)
        x = x + self.attn(self.ln_1(x), self.ln_2(data), pos_emb=pos_emb)
        x = x + self.mlp(self.ln_3(x))
        x = self.seqs2imgs(x,h=H,w=W)
        x = self.upsample(x)
        x = self.conv_out(x)
        # x = self.imgs2seqs(x)
        return x.contiguous()