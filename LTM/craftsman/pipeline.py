...
class CraftsManPipelineOutput(BaseOutput):
    """
...
class CraftsManPipeline():
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], **kwargs):
        ...
        try:
            ...
            warnings.warn(f"Could not compile the model, error {e}")
    ...
    def preprocess_image(
        ...
        for i in range(len(images_pil)):
            ...
            padded_image = PIL.Image.new("RGBA", image.size, (*background_color, 255))
            ...
            new_image = PIL.Image.new("RGBA", new_size, (*background_color, 255))
    ...
    def __call__(
        ...
        preprocessed_images_pil = self.preprocess_image(
            images_pil, 
        ...
        if not output_type == "latent":
            ...
            for i, cur_latents in enumerate(latents):
                ...
                if output_type == "trimesh":
                    ...
                    volume_threshold = 0.01 # remove small components
                    filtered_components = [comp for comp in components if comp.volume > volume_threshold]
                    cur_mesh = trimesh.util.concatenate(filtered_components)
        ...
        if not return_dict:
            return tuple(preprocessed_images_pil), tuple(mesh)
        return CraftsManPipelineOutput(images=preprocessed_images_pil, meshes=mesh)