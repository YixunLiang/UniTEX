'''
continuous remeshing implementation
...
def split_edges(
    ...
    faces = torch.cat((shrunk_faces,new_faces.reshape(F*3,3))) #4F,3
    if pack_faces:
        mask = faces[:,0]!=0
        mask[0] = True
...
def collapse_edges(
    ...
    edge_rank = rank #E
    for i in range(3):
        vert_rank.scatter_reduce_(src=edge_rank[:,None].expand(-1,2).reshape(-1),index=edges.reshape(-1),dim=0,reduce="max",include_self=False)
...
def flip_edges(
    ...
    edges_neighbors = torch.cat((edges[candidates],neighbors[candidates]),dim=-1) #E',4
    _,order = loss_change.sort(descending=True, stable=stable) #E'
    rank = torch.zeros_like(order)
    rank[order] = torch.arange(0,len(rank),device=rank.device)
    vertex_rank = torch.zeros((V,4),dtype=torch.long,device=device) #V,4
    vertex_rank.scatter_reduce_(src=rank[:,None].expand(-1,4),index=edges_neighbors,dim=0,reduce="max",include_self=False)
...
def spatial_smooth(
    ...
    edges,_ = calc_edges(faces) #E,2
    E = edges.shape[0]
    edge_smooth = vertices_etc[edges] #E,2,S
    neighbor_smooth = torch.zeros_like(vertices_etc) #V,S
    neighbor_smooth.scatter_reduce_(src=edge_smooth.flip(dims=[1]).reshape(E*2,-1),index=edges.reshape(E*2,1).expand(-1, edge_smooth.shape[-1]),dim=0,reduce="mean",include_self=False)
    return neighbor_smooth

@torch.no_grad()
def remesh(
    vertices:torch.Tensor, # V,D
    faces:torch.Tensor, # F,3 long
    vertices_etc:Optional[torch.Tensor]=None, # V,D
    min_edgelen:Optional[torch.Tensor]=None, # V
    max_edgelen:Optional[torch.Tensor]=None, # V
    flip=True,
    max_vertices=1e6,
) -> Tuple[torch.Tensor, torch.Tensor]:
    if min_edgelen is None or max_edgelen is None:
        e_dis = compute_edge_length(vertices, faces)
        e_dis_mean = e_dis.mean().item()
        e_dis_std = e_dis.std().item()
        if min_edgelen:
            min_edgelen = torch.full_like(vertices[:, 0], fill_value=max(e_dis_mean-e_dis_std, 0.0))
        if max_edgelen:
            max_edgelen = torch.full_like(vertices[:, 0], fill_value=e_dis_mean+e_dis_std)

    # dummies
    if vertices_etc is not None:
        vertices_total = torch.cat([vertices, vertices_etc],dim=1)
    else:
        vertices_total = vertices
    vertices_total = torch.cat((torch.full_like(vertices_total[[0], :],fill_value=torch.nan),vertices_total),dim=0)
    faces = torch.cat((torch.zeros_like(faces[[0], :]),faces+1), dim=0)
    nan_tensor = torch.tensor([torch.nan],device=min_edgelen.device)
    min_edgelen = torch.cat((nan_tensor,min_edgelen))
    max_edgelen = torch.cat((nan_tensor,max_edgelen))

    # collapse
    edges,face_to_edge = calc_edges(faces) #E,2 F,3
    vertices = vertices_total[:,:3] #V,3
    ...
    vertices_total,faces = collapse_edges(vertices_total,faces,edges,priority)

    # split
    if vertices_total.shape[0]<max_vertices:
        edges,face_to_edge = calc_edges(faces) #E,2 F,3
        vertices = vertices_total[:,:3] #V,3
        edge_length = calc_edge_length(vertices,edges) #E
        splits = edge_length > max_edgelen[edges].mean(dim=-1)
        vertices_total,faces = split_edges(vertices_total,faces,edges,face_to_edge,splits,pack_faces=False)

    # flip
    vertices_total,faces = pack(vertices_total,faces)
    if flip:
        edges,_,edge_to_face = calc_edges(faces,with_edge_to_face=True) #E,2 F,3
        vertices = vertices_total[:,:3]
        flip_edges(vertices,faces,edges,edge_to_face,with_border=False)

    vertices = vertices_total[1:,:3]
    faces = faces[1:,:]-1
    if vertices_etc is not None:
        vertices_etc = vertices_total[1:,3:]
    else:
        vertices_etc = None
    return vertices, faces, vertices_etc
